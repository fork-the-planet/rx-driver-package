...
int aes_setkey_generic(mbedtls_aes_context * ctx, const unsigned char * key, unsigned int keybits)
{
    ...
    switch (keybits)
    {
        ...
        {
            #if BSP_FEATURE_RSIP_SCE9_SUPPORTED || BSP_FEATURE_RSIP_SCE5B_SUPPORTED ||                           \
                                    ...
                                    unsigned char       * output)
{
    ...
    if (ctx->nr == 10)
    {
        ...
        if ((text_bitlen % HW_SCE_AES_BLOCK_BIT_SIZE) == 0)
        {
            ...
            if (err == 0U)
            {
                /* Once final is successful,
                 * change operation state back to SCE_MBEDTLS_CIPHER_OPERATION_STATE_INIT to allow same AES key to
