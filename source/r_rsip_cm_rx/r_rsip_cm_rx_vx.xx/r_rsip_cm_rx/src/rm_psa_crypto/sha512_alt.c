                        ...
                        size_t ilen)
{
    ...
    if (left && ilen >= fill) {
        ...
        {
            if ((ret = mbedtls_internal_sha512_process_ext(ctx, ctx->rsip_buffer,  \
        ...
        if ((ret = mbedtls_internal_sha512_process_ext(ctx, ctx->buffer, \
                                                        SIZE_MBEDTLS_SHA512_PROCESS_BUFFER_BYTES)) != 0)
        {
            return ret;
        }
    ...
    if (ilen >= SIZE_MBEDTLS_SHA512_PROCESS_BUFFER_BYTES)
    {
        ...
        {
            if ((ret = mbedtls_internal_sha512_process_ext(ctx,
                                                            input,
