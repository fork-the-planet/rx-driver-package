...
int ecp_load_parameters_sce(const mbedtls_ecp_group * grp, uint8_t * p_curve_params_buff)
{
    ...
    /* ARM do not define the A field (set to NULL) for R1 curves
     * because they calculate portions of it on the fly to optimize their math.
    ...
    /* The arrays for a, b, p, n, Gx, Gy will be added in that order after reversing each individually.
     * p, n, Gx, Gy fields are of size curve_bytes = PSA_BITS_TO_BYTES( ecp->grp.pbits )
     * For "a" and "b", ARM has provided different sized values depending on the curve even though
     * the sizes are same just that the undefined values are all 0s.
