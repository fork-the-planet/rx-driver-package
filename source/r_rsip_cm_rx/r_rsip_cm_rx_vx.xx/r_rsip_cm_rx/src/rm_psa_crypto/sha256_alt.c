...
/* Re-consider above after https://reviews.llvm.org/D131064 merged.
 *
...
#      elif defined(__GNUC__)
        /*        GCC 5 claims to support Armv8 Crypto Extensions, but some
                        ...
                        size_t ilen)
{
    ...
    if (left && ilen >= fill) {
        ...
        {
            if((ret = mbedtls_internal_sha256_process_ext(ctx, ctx->rsip_buffer, \
                                SIZE_MBEDTLS_SHA256_PROCESS_BUFFER_BYTES)) != 0)
            {
                return ret;
            }
        ...
        if((ret = mbedtls_internal_sha256_process_ext(ctx, ctx->buffer, \
                                SIZE_MBEDTLS_SHA256_PROCESS_BUFFER_BYTES)) != 0)
        {
            return ret;
        }
    ...
    if (ilen >= SIZE_MBEDTLS_SHA256_PROCESS_BUFFER_BYTES)
    {
        ...
        else
        {
            ...
            uint32_t final_block_start = (sha256_block_aligned_size_mod * SIZE_MBEDTLS_SHA256_PROCESS_BUFFER_BYTES) - \
                                            SIZE_MBEDTLS_SHA256_PROCESS_BUFFER_BYTES;
        ...
        {
            if((ret = mbedtls_internal_sha256_process_ext(ctx, input, \
                        sha256_block_aligned_size_mod * SIZE_MBEDTLS_SHA256_PROCESS_BUFFER_BYTES)) != 0 )
                return(ret);
...
/* Update the HW sce operation status/rsip command */
    ...
    if (ilen >= SIZE_MBEDTLS_SHA256_PROCESS_BUFFER_BYTES)
    {
        ...
        if((ret = mbedtls_internal_sha256_process_ext(ctx, input,                               \
                    sha256_block_aligned_size * SIZE_MBEDTLS_SHA256_PROCESS_BUFFER_BYTES)) != 0)
            return(ret);
