                                    ...
                                    const uint32_t * InData_S)
{
    ...
    /* 119bit 0 padding + R + 119bit 0 padding + S */
    memcpy(&signature[padding], InData_R, ((HW_SCE_ECDSA_P521_DATA_BYTE_SIZE / 2U) - padding));
    memcpy(&signature[((HW_SCE_ECDSA_P521_DATA_BYTE_SIZE / 2U)) + padding], InData_S,
            ((HW_SCE_ECDSA_P521_DATA_BYTE_SIZE / 2U) - padding));

    /* OEM key install expects 44words of Public key as input */
    uint8_t   Pubkey[HW_SCE_ECDSA_P521_DATA_BYTE_SIZE + 16] = {0};
    uint8_t * IndataPublickey = (uint8_t *) InData_PubKey;
    /* 119bit 0 padding + Px + 119bit 0 padding + Py */
    memcpy(&Pubkey[padding], IndataPublickey, ((HW_SCE_ECDSA_P521_DATA_BYTE_SIZE / 2U) - padding));
