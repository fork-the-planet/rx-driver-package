                                ...
                                mbedtls_ecp_restart_ctx * rs_ctx)
{
    ...
    else if (0 != mbedtls_mpi_write_binary(&P->X, (((uint8_t *)p_point_buff_P_32) + padding), curve_bytes))
    {
        ret = MBEDTLS_ERR_ECP_ALLOC_FAILED;
    }
    /* Write Py into the buffer in reverse */
    else if (0 != mbedtls_mpi_write_binary(&P->Y, (((uint8_t *)p_point_buff_P_32) + (curve_bytes + (padding * 2))),
                                    curve_bytes))
    {
        ret = MBEDTLS_ERR_ECP_ALLOC_FAILED;
    }
    ...
    else if (0 != mbedtls_mpi_read_binary(&R->X, ((uint8_t *)p_point_result_buff_R_32) + padding, curve_bytes))
    {
        ret = MBEDTLS_ERR_MPI_ALLOC_FAILED;
    }
    else if (0 != mbedtls_mpi_read_binary(&R->Y, (((uint8_t *)p_point_result_buff_R_32) + curve_bytes + (padding * 2)),
                                    curve_bytes))
    {
        ret = MBEDTLS_ERR_MPI_ALLOC_FAILED;
    }
